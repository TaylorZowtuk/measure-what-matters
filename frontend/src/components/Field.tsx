...
class Field extends React.Component 
    ...
    removeFromField = (player:Player | undefined): void => {
        if (player === undefined) {
            ...
            var index = array.findIndex(playerDraggable => playerDraggable.number === player.num);
            if (index !== -1) {
    ...
    componentDidUpdate(prevProps: any, _prevState: any) {
        if (this.props.removeFromField !== undefined && this.props.addToField !== undefined) {
            this.substitute();
        }
    }
    ...
    render () {
        return (
            <FieldTarget draggablePlayers={this.state.onField} incrementScore={this.props.incrementScore}/>
        )
    }
}
